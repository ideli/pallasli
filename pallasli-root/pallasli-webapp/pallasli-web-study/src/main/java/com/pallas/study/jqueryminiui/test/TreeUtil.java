package com.pallas.study.jqueryminiui.test;
...
public class TreeUtil {
	...
	public static ArrayList ToTree(List table, String childrenField,
	...
	private static void SyncTreeNodes(ArrayList nodes, int outlineLevel,
			String outlineNumber, String childrenField) {
		for (int i = 0, l = nodes.size(); i < l; i++) {
			...
			if (childNodes != null && childNodes.size() > 0) {
				SyncTreeNodes(childNodes, outlineLevel + 1,
						node.get("OutlineNumber").toString() + ".",
						childrenField);
			}
		}
	}

	public static ArrayList ToList(List tree, String parentId,
			String childrenField, String idField, String parentIdField) {
		...
		for (int i = 0, len = tree.size(); i < len; i++) {
			...
			if (children != null && children.size() > 0) {
				List list2 = ToList(children, task.get(idField).toString(),
						childrenField, idField, parentIdField);
				list.addAll(list2);
			}
