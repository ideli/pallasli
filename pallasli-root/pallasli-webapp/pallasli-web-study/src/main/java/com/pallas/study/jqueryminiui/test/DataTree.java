...
public class DataTree {
	...
	public void Load(ArrayList tree) {
		...
		for (int i = list.size() - 1; i >= 0; i--) {
			...
			node.put(leafField, (nodes == null || nodes.size() == 0) ? true
					: false);
			...
			if (filtered != null) {
				...
				for (int i = 0, l = filtered.size(); i < l; i++) {
					...
					for (int j = 0, k = ans.size(); j < k; j++) {
						...
						if (filterMaps.get(pid) == null)
							filterMaps.put(pid, pnode);
					}
				}
	...
	protected Boolean IsVisibleNode(HashMap node) {
		...
		if (parent == null)
			return true;
		if (parent.get(expandedField) == null)
			return IsVisibleNode(parent);
		if ((Boolean) parent.get(expandedField) == false)
			return false;
		return true;
	}
