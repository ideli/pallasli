...
public class BirtEngineServlet extends BaseReportEngineServlet {
	...
	protected void __doGet(IContext context) throws ServletException, IOException, BirtException {
		...
		if (((("/preview".equalsIgnoreCase(context.getRequest().getServletPath()))
				|| ("/document".equalsIgnoreCase(context.getRequest().getServletPath()))
				|| ("/output".equalsIgnoreCase(context.getRequest().getServletPath()))))
				&& (bean.isShowParameterPage())) {
			this.requester.service(context.getRequest(), context.getResponse());
		} else if ("/parameter".equalsIgnoreCase(context.getRequest().getServletPath())) {
			this.requester.service(context.getRequest(), context.getResponse());
		} else {
