...
public class SingleTaskTest extends PluggableActivitiTestCase {
	...
	public void testProcess() {
		if (true)
			return;
		InputStream jsonStream = this.getClass().getClassLoader().getResourceAsStream("singleTask.json");
		...
		Deployment deployment = repositoryService.createDeployment().addString("lyttest.bpmn", xml).deploy();
		int i = 0;
		...
		processInstance = runtimeService.startProcessInstanceByKey("lyttest", map);
