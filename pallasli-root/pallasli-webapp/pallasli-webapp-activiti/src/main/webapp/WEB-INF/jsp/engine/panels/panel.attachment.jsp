...
Ext.onReady(function(){
	...
	var store = new Ext.data.DirectStore({
		...
		directFn : eval(app.keyPrefix + 'AppDirect.getDocumentPanelFileList'),
...
%>
	...
	panel.uploadfile = function() {
		var dialog = new Ext.ux.UploadDialog.Dialog({
			url: app.url + '/sysupload.do',
			modal : true,
		...
		dialog.on('uploadsuccess', panel.refresh, this);
		dialog.show();
	}

	panel.downloadfile = function() {
		var record = grid.getSelectionModel().getSelected();
		location.href = app.url + '/sysdownload.do?documentdbtype=blob&fieldname=F_CONTENT&tablename=t_mk_sys_files&id='+record.get('ID')+'&filename='+record.get('F_ATTACH_NAME');
	}
