...
<%@page import="com.mixky.engine.organization.UserExpression"%>
<%@ page import="com.mixky.engine.workflow.transaction.ClientData"%>
<%@ page import="com.mixky.engine.design.workflow.Node"%>
<%
	// 读取参数
	...
	if (clientdata.users != null) {
		for (int i = 0; i < clientdata.users.size(); i++) {
			...
			userjson.addProperty("iconCls", "icon-app-user");
			userjson.addProperty("type", "user");
			userjson.addProperty("leaf", true);
			userjson.addProperty("expression", UserExpression.getExpression(clientdata.users.get(i)).getExpressionString());
	...
	if (clientdata.processors != null) {
		for (int i = 0; i < clientdata.processors.size(); i++) {
			...
			userjson.add(new JsonPrimitive(UserExpression.getExpression(clientdata.processors.get(i)).getExpressionString()));
	...
	if (clientdata.assistants != null) {
		for (int i = 0; i < clientdata.assistants.size(); i++) {
			JsonArray userjson = new JsonArray();
			userjson.add(new JsonPrimitive(UserExpression.getExpression(clientdata.assistants.get(i)).getExpressionString()));
	...
	if (clientdata.readers != null) {
		for (int i = 0; i < clientdata.readers.size(); i++) {
			JsonArray userjson = new JsonArray();
			userjson.add(new JsonPrimitive(UserExpression.getExpression(clientdata.readers.get(i)).getExpressionString()));
...
Ext.onReady(function(){
        	...
        	items : [{
        		...
        		handler : function(){
					...
					if(tab == gridprocessors && !<%=selectMulti%>){
						if(store.getCount() > 0 || node.attributes.type != 'user'){
							MixkyApp.showAlertMessage('只允许单人办理！');
							return;
						}
					}
	...
	var gridprocessors = new Ext.grid.GridPanel({
		...
		contextMenu : new Ext.menu.Menu({
		    items : [{
        		text : '取消',
        		handler : function(){
        		    var cm = gridprocessors.getSelectionModel();
					var record = cm.getSelected();
					if(record){
						var index = storeprocessors.indexOf(record);
	...
	var gridassistants = new Ext.grid.GridPanel({
		...
		contextMenu : new Ext.menu.Menu({
		    items : [{
        		text : '取消',
        		handler : function(){
        		    var cm = gridassistants.getSelectionModel();
					var record = cm.getSelected();
					if(record){
						var index = storeassistants.indexOf(record);
	...
	var gridreaders = new Ext.grid.GridPanel({
		...
		contextMenu : new Ext.menu.Menu({
		    items : [{
        		text : '取消',
        		handler : function(){
        		    var cm = gridreaders.getSelectionModel();
					var record = cm.getSelected();
					if(record){
						var index = storereaders.indexOf(record);
	...
	panel.doLayout();
	// 设置状态
	panel.setpWindow.step = Mixky.wasoft.workflow.STEP_USERSELECT;
	// 设置按钮
