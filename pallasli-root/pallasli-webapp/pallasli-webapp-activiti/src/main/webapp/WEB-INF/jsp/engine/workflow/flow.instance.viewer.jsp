...
<%@ page import="com.mixky.engine.design.workflow.Flow"%>
...
<%
	...
	if(flowlogId > 0){
		List<NodeLog> nlogs = InstanceDataService.instance().getNodeLogs(flowlogId, null, -1);
	...
	String wfiFlowCellDirect = "eval(app.keyPrefix + 'AppDirect.getFlowCells')";
	String wfiOpinionLogDirect = "eval(app.keyPrefix + 'AppDirect.getUserOpinionLog')";
%>
...
Ext.onReady(function(){
	...
	function getNodeType(id, isCompute){
		...
		return isCompute ? "nodeComputeUnpassed" : "nodeDone";
	}
	...
	panel.refresh = function(){
		...
		var wfiDirect = eval(app.keyPrefix + 'AppDirect');
		wfiDirect.getFlowCells('<%=workflow.getKey()%>', false, function(result, e){
			if(result && result.success){
				...
				try
				{
					...
					for(var i=0;i<result.nodes.length;i++){
						...
						}else if(type == 'nodeCompute'){
						    w = 100;
						    h = 35;
						    type = getNodeType(node.id, true);
						}
					...
					for(var i=0;i<result.routes.length;i++){
						...
						if(route.style){
							// 设置线条位置
							var geo = edge.getGeometry();
							geo.points = [new mxPoint(route.style.point.x * (140/180), route.style.point.y * (80/100))];
							edge.setGeometry(geo);
							// 设置线条样式
