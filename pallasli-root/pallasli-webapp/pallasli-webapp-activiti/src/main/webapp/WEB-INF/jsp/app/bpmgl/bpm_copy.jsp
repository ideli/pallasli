...
<%
    ...
    String name = request.getParameter("name") + "（复制）";
...
Ext.onReady(function(){
	...
	var id = {"name":"id","xtype":"hidden",value:'<%=id%>'};
	
	var name = {"xtype":"textfield","anchor":"100%","inputValue":"","selectOnFocus":true,"name":"name","fieldLabel":"名称","allowBlank":false,"labelStyle":"color:red",value:'<%=name%>'};
	var description = {"xtype":"textarea","anchor":"100%","inputValue":"","selectOnFocus":true,"name":"description","fieldLabel":"描述","allowBlank":true,"maxLength":500,height:200,value:'<%=description%>'};
	...
	panel.submit = function(){
		   ...
		   //if(form.getForm().isDirty()){	//不管表单是否编辑，都走后台 张长龙
		   Ext.MessageBox.confirm('提示', '您确定要复制保存该流程吗?', function(btn){
	    		if(btn == 'yes'){
				   var params = form.getForm().getFieldValues();
		           BpmFlowDirect.copyProcess(params,function(result,e){
