...
Mixky.editor.SysFiles = Ext.extend(Ext.form.TextField, {
	...
	loadFiles:function(){
		...
		var fn = eval(app.keyPrefix + 'AppDirect.getDocumentFileList');
	...
	downloadFile : function(id){
		location.href = "file/download.do?id=" + id + "&fk=" + this.name + "&dk=" + this.document.document.documentkey;
	},
	
	deleteFile : function(id){
		var fileEl = this;
		var app = Mixky.wasoft.cache.Applications[this.applicationkey];
		var fn = eval(app.keyPrefix + 'AppDirect.deleteDocumentFile');
		fn(id, function(result, e){
			if(result && result.success) {
				fileEl.loadFiles();
			} else {
