...
Mixky.editor.FileField = Ext.extend(Ext.form.DisplayField, {
	...
	addFileInput : function(){
		this.curFileInput = this.el.createChild({
            ...
            style: 'position: absolute;-moz-opacity: 0;filter:alpha(opacity: 0);opacity: 0;z-index: 2; height: 22px; width: '+this.width + 'px;',
            tag: 'input',
            type: 'file',
            size: 1
        });
    ...
    addFileDisplay : function(fileId, filename, fileSize, fileInput){
	    ...
	    if(fileId > 0){
			// 从数据库中装入
			var url = this.downloadUrl != '#' ? this.downloadUrl + '&id=' + fileId + '&fn=' + filename : '#';
			if(this.isImage){
