...
Mixky.editor.showOrganizationWindow = function(fn, initvalues, cfg){
	...
	var store = new Ext.data.DirectStore({
    	...
    	paramOrder : ['limit','start','expression'],
		baseParams:{limit:60, start:0, key:'expression'},
		root : 'results',
	...
	var selectedbox = new Ext.DataView({
        ...
        tpl:new Ext.XTemplate(
        	'<tpl for=".">',
                '<div class="user-expression-item icon-administrator-{type}" id="{expression}">{f_caption}</div>',
            '</tpl>',
        ...
        store : new Ext.data.JsonStore({
        	...
        	fields: ['expression', 'type', 'id', 'f_name', 'f_caption']
        }),
        // 选中表达式
        selectExpression:function(expression){
			...
			// 服务器端解析表达式
			OrganizationDirect.getExpressionData1(expression, config.selectType, function(result,e){
				// 添加
