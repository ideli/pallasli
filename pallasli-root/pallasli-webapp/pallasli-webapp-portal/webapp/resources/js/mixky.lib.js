...
Mixky.lib.getNewTableRecordId = function(tablename, fn){
...
Mixky.lib.alertObject = function(obj){
	var str = 'item is \n';
	for(var n in obj){
		str = str + '\t' + n + ":" + obj[n] + "\n";
	}
...
Mixky.lib.IframeManager = {
	loadUrl : function(url){
		if(!Mixky.lib.IframeManager.el){
			var iframe = document.createElement("iframe");
			iframe.setAttribute("width","0px");
			iframe.setAttribute("height","0px");
			Mixky.lib.IframeManager.el = Ext.getBody().appendChild(iframe);
		}
		Mixky.lib.IframeManager.el.dom.src = url;
	}
}
