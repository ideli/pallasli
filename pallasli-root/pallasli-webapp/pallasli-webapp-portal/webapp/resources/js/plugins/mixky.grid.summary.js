Mixky.plugins.GridSummary = function(config){
    Ext.apply(this, config);
};

Ext.extend(Mixky.plugins.GridSummary, Ext.util.Observable, {
    init : function(grid){
        this.grid = grid;
        ...
        for(var j = 0, jlen = rs.length; j < jlen; j++){
            ...
            for(var i = 0, len = cs.length; i < len; i++){
                ...
                if(cf && cf.summaryType){
                    data[c.name] = Mixky.plugins.GridSummary.Calculations[cf.summaryType](data[c.name] || 0, r, c.name, data);
                }
            }
        }
        return data;
...
Mixky.plugins.GridSummary.Calculations = {
...
Mixky.plugins.HybridGridSummary = Ext.extend(Mixky.plugins.GridSummary, {
    calculate : function(rs, cs){
        var gcol = this.view.getGroupField();
        var gvalue = rs[0].data[gcol];
        var gdata = this.getSummaryData(gvalue);
        return gdata || Mixky.plugins.HybridGridSummary.superclass.calculate.call(this, rs, cs);
    },
