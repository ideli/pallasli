	...
	if (objid != "") {
		...
		if (obj) {
			try {
				...
				obj.ActiveDocument.TrackRevisions = trackrevisions;
				//obj.ShowDocumentRevision(showrevisions);
				obj.ActiveDocument.ShowRevisions = trackrevisions;
	...
	if (objid != "") {
		...
		if (obj) {
			try {
				...
				obj.ActiveDocument.Application.UserName = mixap_doc_original_username;
				obj.close();
			} catch (e) {
				//alert(e.message);
			}
...
function showWordDocumentRevision(objid, show) {
	if (objid != "") {
		...
		if (obj) {
			...
			obj.ActiveDocument.ShowRevisions = show;
		}
	}
}
