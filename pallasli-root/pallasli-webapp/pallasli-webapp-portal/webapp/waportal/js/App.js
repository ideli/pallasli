Ext.define('Pallas.portal.waapp.App', {
	...
	initApp : function() {
		...
		// 创建桌面对象
		this.desktop = new Pallas.portal.waapp.desktop.webpage.Desktop(this);
	...
	hasShortcut : function(btntype, appkey, key) {
		for (var i = 0; i < this.userConfig.shortcuts.length; i++) {
			...
			if (sc.btntype == btntype && sc.applicationkey == appkey
					&& sc.key == key) {
				return true;
			}
		}
	...
	hasQuickStart : function(btntype, appkey, key) {
		for (var i = 0; i < this.userConfig.quickstarts.length; i++) {
			...
			if (qs.btntype == btntype && qs.applicationkey == appkey
					&& qs.key == key) {
				return true;
			}
		}
	...
	doAutoOpen : function() {
		...
		if (Ext.isDefined(this.openParams)) {
			if (Ext.isDefined(this.openParams.modulekey)) {
				var module = this.desktop.openModule(this.openParams.appkey,
						this.openParams.modulekey);
				if (this.openParams.viewkey && Ext.isDefined(module)) {
					module.openView(this.openParams.viewkey);
				}
			}
			if (Ext.isDefined(this.openParams.documentkey)
					&& Ext.isDefined(this.openParams.documentid)) {
				isOpenDocument = true;
				Mixky.wasoft.lib
						.openFavorite(this.openParams.appkey,
								this.openParams.documentkey,
								this.openParams.documentid);
			}
		}
	},
	// 添加桌面按钮
