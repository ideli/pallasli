Ext.define('Pallas.portal.waapp.lib.Cache', {
	statics : {

		dictionaryRenderer : function(appkey, val, dictionaryname) {
			var dictionary = Mixky.wasoft.lib.cache.getDictionary(appkey,
					dictionaryname);
			var display = val;
			if (Ext.isDefined(dictionary)) {
				dictionary.each(function(record) {
					if (record.get('display') == val
							|| record.get('value') == val) {
		...
		getDictionaryUrlRender : function(appkey, url, value, fn) {
		...
		getDictionaryDourlRender : function(appkey, url, value, fn, params) {
			var pageurl = 'page.do';
			if (Ext.isDefined(appkey) && appkey != '') {
				var app = Mixky.wasoft.cache.Applications[appkey];
		...
		getDictionaryUrlStore : function(appkey, url) {
			if (Ext.isDefined(appkey) && appkey != '') {
				var app = Mixky.wasoft.cache.Applications[appkey];
				url = app.url + '/' + url;
			}
		...
		getDictionaryDoUrlStore : function(appkey, url, params) {
			var app = Mixky.wasoft.cache.Applications[appkey];
			var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : app.url + "/page.do"
		...
		getDictionaryDirectStore : function(fn, params, panel) {
