...
Ext.onReady(function(){
	...
	// 获得对象的属性列表
	var parentModule = Mixky.awsoft.lib.Class.getModule(mclass);
	var module = Mixky.awsoft.lib.Class.getModule(type);
	...
	// 合并属性
	var properties = {};
	for(var i=0;i<Mixky.awsoft.lib.Class.defaultProperties.length;i++){
		var p = Mixky.awsoft.lib.Class.defaultProperties[i];
		properties[p.name] = p;
	}
	...
	var renderer = function(value, p, record){
		...
		return String.format("<div style='height:16px;padding-left:23px;background:transparent url(resources/icon/designer/{0}.gif) no-repeat'> {1}</div>", type, value);
	}
	// 列表字段
	...
	var store = new Ext.data.DirectStore({
		...
		directFn : DesignObjectDirect.getSubObjectList,
    ...
    var AddAction = new Ext.Action({
		...
		handler:function(){
			...
			Mixky.awsoft.lib.addDesignObject(key, type, function(newkey, mclass){
				panel.refresh();
			});
		}
	});
    var CopyAction = new Ext.Action({
		...
		handler:function(){
			...
			Mixky.awsoft.lib.Context.clipboardObject = {key:record.get('key'), mclass:type};
		}
	});
    var PasteAction = new Ext.Action({
		...
		handler:function(){
			...
			DesignObjectDirect.pasteObject(srcoid.key, dstoid.key, function(result, e){
				if(result.success){
					panel.refresh();
				}else{
		...
		handler:function(){
			...
			Ext.MessageBox.confirm('危险操作提示', '删除当前选中的对象，该操作不可恢复，您确定吗？', function(btn){
				if(btn == 'yes'){
					...
					DesignObjectDirect.deleteObjects(delKeys, function(result, e){
						if(result.success){
							panel.refresh();
						}
					});
	...
	panel.save = function(needSaveNext){
		...
		if(modifieds.length > 0){
			DesignObjectDirect.saveObject(modifieds[0].get('key'), modifieds[0].getChanges(), function(result, e){
				if(result.success){
			...
			Mixky.framework.workspace.savePanelOver(panel, needSaveNext);
		}
	};
	panel.add(ui);
	panel.doLayout();
