...
public class TestDbMethod {
	...
	public void testExecute() throws CallDbException, Exception {
		...
		// dbMethod.Execute(null, procedureName, method.invoke(testExeCase,
		// null));
	}
	...
	public void testOpen() throws CallDbException, Exception {
		...
		// List lst = dbMethod.Open(procedureName, method.invoke(testOpenCase,
		// null));
		// displayList(lst);
	}
	...
	public void testDbMethod() throws Exception {
		try {
			...
			// DataSource ds = new GjjmxDataSource(0,
			// "weblogic.jndi.WLInitialContextFactory",
			// "t3://localhost:7001",
			// "weblogic", "weblogic", "jdbc/ggj", "gjj");

			// DataSource ds = new
			// GjjmxDataSource("oracle.jdbc.driver.OracleDriver",
			// "jdbc:oracle:thin:@192.168.0.115:1521:orcl",
			// "gjjmx12_ta","adminconnect");
			DataSource ds = new GjjmxDataSource(
					"com.ibm.as400.access.AS400JDBCDriver",
					"jdbc:as400://192.168.0.20:50000/dq", "jtpadmin",
					"adminconn");
