...
/**
 * NRT（near real time）近实时搜索测试 近实时搜索(Near real time
 * search)表示当索引数据发生变化时，Lucene能及时把这种变化反映到客户进行查询的结果中。 实现原理： 只有Index
 * Writer上的commit操作才会导致ram directory上的数据完全同步到文件。Index
 * Writer提供了实时获得reader的API，这个调用将导致flush操作，生成新的segment，但不会commit（fsync），从而减少
 * 了IO。新的segment被加入到新生成的
 * reader里。从返回的reader里，可以看到更新。所以，只要每次新的搜索都从IndexWriter获得一个新的reader
 * ，就可以搜索到最新的内容。这一操作的开销仅仅是flush，相对commit来说，开销很小。
 * Lucene的index组织方式为一个index目录下的多个segment
 * 。新的doc会加入新的segment里，这些新的小segment每隔一段时间就合并起来。因为合并，总的segment数
 * 量保持的较小，总体search速度仍然很快
 * 。为了防止读写冲突，lucene只创建新的segment，并在任何active的reader不在使用后删除掉老的segment。
 * flush是把数据写入到操作系统的缓冲区，只要缓冲区不满，就不会有硬盘操作，相对轻量级操作。
 * commit是把所有内存缓冲区的数据写入到硬盘，是完全的硬盘操作，重量级操作。
 * 
 * @author Johnny
 * 
 */
public class NearRealTimeSearch {
	...
	private final String[] ids = { "1", "2", "3", "4", "5", "6" };
	private final String[] emails = { "aa@itat.org", "bb@itat.org",
	...
	private final int[] attachs = { 2, 3, 1, 4, 5, 5 };
	private final String[] names = { "zhangsan", "lisi", "john", "jetty",
			"mike", "jake" };
	...
	public NearRealTimeSearch() {
		...
		try {
			...
			// "/Users/ChinaMWorld/Desktop/index/"));
			// directory = FSDirectory.open(new File("E://FSDIrectory/index/"));
			directory = FSDirectory.open(new File("FSDIrectory/index/"));
