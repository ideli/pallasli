...
public class SessionScanerListener implements HttpSessionListener,
	...
	/**
	 * @Field: list 定义一个集合存储服务器创建的HttpSession LinkedList不是一个线程安全的集合
	 */
	/**
	 * private List<HttpSession> list = new LinkedList<HttpSession>();
	 * 这样写涉及到线程安全问题,SessionScanerListener对象在内存中只有一个 sessionCreated可能会被多个人同时调用，
	...
	public void sessionCreated(HttpSessionEvent se) {
		...
		synchronized (lock) {
			 ...
			 * 而这把锁正在被往集合中添加session的那个thread
			 * -1占用着，因此thread-2只能等待thread-1操作完成之后才能够进行操作
			 * 当thread-1添加完session之后，就把lock放开了
			 * ，此时thread-2拿到lock，就可以执行遍历list集合中的session的那段代码了
...
class MyTask extends TimerTask {
	...
	public void run() {
		...
		synchronized (lock) {
			 ...
			 * 用户一访问，服务器就会为该用户创建一个session
			 * ,此时就会调用sessionCreated往list集合中添加新的session，
			 * 然而定时器在定时执行扫描遍历list集合中的session时是无法知道正在遍历的list集合又添加的新的session进来了，
			 * 这样就导致了往list集合添加的新的session和遍历list集合中的session这两个操作无法达到同步
			 * 那么解决的办法就是把"list.add(session)和while(it.hasNext()){//迭代list集合}"
			 * 这两段代码做成同步， 保证当有一个线程在访问"list.add(session)"这段代码时，另一个线程就不能访问
			 * "while(it.hasNext()){//迭代list集合}"这段代码
