package com.pallasli.httpclent.examle;
 ...
 * 
 * @author 杨志龙 blog:http://www.cnblogs.com/yangzhilong
 *
 */
public class ReadCodeFromWeb {
	...
	public static void readShi(String url, BufferedWriter bw) throws Exception {
		...
		// '><TD><A HREF='11/1101.HTML'>110100000000</A></TD><TD><A
		// HREF='11/1101.HTML'>市辖区</A></TD></td><TR CLASS='
		for (int c = 1, len = citys.length; c < len; c++) {
			...
			for (int si = 1; si < 3; si++) {
				if (si == 1) {// 取链接和编码
					...
					String cityCode = strs[si].substring(
							strs[si].indexOf("'>") + 2,
							strs[si].indexOf("</A>"));
					...
					bw.write("<td></td><td>");
					bw.write(strs[si].substring(strs[si].indexOf("'>") + 2,
							strs[si].indexOf("</A>")));
					bw.write("</td><td></td><td></td><td></td><td></td></tr>");

					System.out.println("爬取:"
							+ strs[si].substring(strs[si].indexOf("'>") + 2,
									strs[si].indexOf("</A>")));
				}
			}
			bw.newLine();
	 ...
	 * @throws Exception
	 */
	public static void readXian(String prix, String url, BufferedWriter bw)
			throws Exception {
		String content = getContent(baseUrl + url).toUpperCase();
		...
		for (int i = 1; i < citys.length; i++) {
			...
			if (citys[i].indexOf("<A HREF='") == -1) {
				...
				bw.write(citys[i].substring(citys[i].indexOf("</TD><TD>") + 9,
						citys[i].lastIndexOf("</TD>")));
	 ...
	 * @throws Exception
	 */
	public static void readZhen(String prix, String url, BufferedWriter bw)
			throws Exception {
		String content = getContent(baseUrl + prix + url).toUpperCase();
		String myPrix = (prix + url).substring(0,
				(prix + url).lastIndexOf("/") + 1);
	 ...
	 * @throws Exception
	 */
	public static void readCun(String prix, String url, BufferedWriter bw)
			throws Exception {
		String content = getContent(baseUrl + prix + url).toUpperCase();
