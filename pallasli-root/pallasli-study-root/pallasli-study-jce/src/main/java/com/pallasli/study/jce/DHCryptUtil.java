...
public class DHCryptUtil {
	...
	public static void main(String[] args) {
		try {
			...
			// 由甲方公钥，乙方私钥构建密文
			byte[] aCode = DHCryptUtil.encrypt(aInput.getBytes(), aPublicKey,
					bPrivateKey);

			// 由乙方公钥，甲方私钥解密
			byte[] aDecode = DHCryptUtil
					.decrypt(aCode, bPublicKey, aPrivateKey);
			...
			// 由乙方公钥，甲方私钥构建密文
			byte[] bCode = DHCryptUtil.encrypt(bInput.getBytes(), bPublicKey,
					aPrivateKey);

			// 由甲方公钥，乙方私钥解密
			byte[] bDecode = DHCryptUtil
					.decrypt(bCode, aPublicKey, bPrivateKey);
