package pallasli.study.mq.activemq.openwire.TopicPubSubSamples.RequestReply;
...
public class TopicReplier implements javax.jms.MessageListener {
	...
	private void start(String broker, String username, String password, String mode) {
		...
		try {
			...
			replier = session.createProducer(null); // Topic will be set for
													// each reply
	 ...
	 * interface).
	 *
	 * IMPORTANT NOTE: We must follow the design paradigm for JMS synchronous
	 * requests. That is, we must: - get the message - look for the header
	 * specifying JMSReplyTo - send a reply to the topic specified there.
