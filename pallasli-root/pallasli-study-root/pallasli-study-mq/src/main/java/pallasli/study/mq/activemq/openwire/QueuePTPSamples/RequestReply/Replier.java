package pallasli.study.mq.activemq.openwire.QueuePTPSamples.RequestReply;
...
public class Replier implements javax.jms.MessageListener {
	...
	private void start(String broker, String username, String password, String rQueue, String mode) {
		...
		try {
			...
			replier = session.createProducer(null); // Queue will be set for
													// each reply
	 ...
	 * interface).
	 *
	 * IMPORTANT NOTES: (1)We must follow the design paradigm for JMS
	 * synchronous requests. That is, we must: - get the message - look for the
	 * header specifying JMSReplyTo - send a reply to the queue specified there.
	 * Failing to follow these steps might leave the originator of the request
	 * waiting forever. (2)Unlike the 'Talk' sample and others using an
	 * asynchronous message listener, it is possible here to use ONLY ONE
	 * SESSION because the messages being sent are sent from the same thread of
	 * control handling message delivery. For more information see the JMS spec
	 * v1.0.2 section 4.4.6.
	 *
	 * OPTIONAL BEHAVIOR: The following actions taken by the message handler
	 * represent good programming style, but are not required by the design
	 * paradigm for JMS requests. - set the JMSCorrelationID (tying the response
	 * back to the original request. - use transacted session "commit" so
	 * receipt of request won't happen without the reply being sent.
	 *
	 */
	@Override
