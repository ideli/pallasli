package pallasli.study.mq.activemq.openwire.TopicPubSubSamples.TransactedChat;
...
public class TransactedChat implements javax.jms.MessageListener {
	...
	private void chatter(String broker, String username, String password) {
		...
		try {
			...
			// but not affect messages consumed. Therefore, we need two
			// sessions.
			...
			while (true) {
				if (showMessage) {
					...
					System.out.println("The application user " + username + " connects to the broker at "
							+ DEFAULT_BROKER_NAME + ".");
					System.out.println("The application will stage messages to the " + APP_TOPIC
				...
				if (s == null)
					...
					if (s.trim().equals("COMMIT")) {
						...
						showMessage = false; // dont't show the help message
												// again.
					} else {
