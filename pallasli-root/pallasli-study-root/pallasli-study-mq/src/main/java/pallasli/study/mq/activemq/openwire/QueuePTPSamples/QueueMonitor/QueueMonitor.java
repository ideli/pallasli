package pallasli.study.mq.activemq.openwire.QueuePTPSamples.QueueMonitor;
...
public class QueueMonitor extends JFrame {
	...
	public QueueMonitor() {
		...
		scrollPane.setBorder(new CompoundBorder(new EmptyBorder(6, 6, 6, 6), new SoftBevelBorder(BevelBorder.LOWERED)));
	...
	private void loadProperties() {
		try {
			...
			System.out.println(propertiesFile + " not found: using defaults"); // Use
																				// Defaults
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (theQueues.size() == 0) {
				...
				for (int i = 0; i < theQueues.size(); i++) {
					try {
						// Create a browser on the queue and show the messages
						// waiting in it.
						...
						if (!e.hasMoreElements()) {
							...
							while (e.hasMoreElements()) {
								...
								if (message != null) {
									...
									try {
										// Scroll the text area to show the
										// message
	...
	public String getContents(javax.jms.Message message) {
		...
		if (message instanceof javax.jms.TextMessage) {
			...
			System.out.println(
					"Warning: A bytes message was discarded because it could not be processed as a javax.jms.TextMessage.");
		} else if (message instanceof javax.jms.ObjectMessage) {
			System.out.println(
					"Warning: An object message was discarded because it could not be processed as a javax.jms.TextMessage.");
		}

		else if (message instanceof javax.jms.StreamMessage) {
			System.out.println(
					"Warning: A stream message was discarded because it could not be processed as a javax.jms.TextMessage.");
		}
