...
public class Server {
	...
	private final ThreadPoolExecutor threadPool = new ThreadPoolExecutor(10,
			10, 500, TimeUnit.MILLISECONDS,
			new ArrayBlockingQueue<Runnable>(20));

	private final Map<Integer, SelectionKey> selectionKeyMap = new ConcurrentHashMap<>();
	private final Map<Integer, List<Object>> responseMessageQueue = new ConcurrentHashMap<>();
						...
						while (iterator.hasNext()) {
							...
							try {
								if (selectedKey.isReadable()) {
									...
									if (selectionKeyMap.get(selectedKey
											.hashCode()) != selectedKey) {
										selectionKeyMap.put(
												selectedKey.hashCode(),
												selectedKey);
										threadPool
												.execute(new ReadClientSocketHandler(
														selectedKey));
									}

								} else if (selectedKey.isWritable()) {
									SocketChannel serverSocketChannel = (SocketChannel) selectedKey
											.channel();
									selectedKey
											.interestOps(SelectionKey.OP_READ);
									List<Object> list = responseMessageQueue
											.get(selectedKey.hashCode());
									if (list == null) {
										list = new LinkedList<Object>();
										responseMessageQueue.put(
												selectedKey.hashCode(), list);
									}
									while (list.size() > 0) {
		...
		public void run() {
			...
			try {
				...
				System.out.println("server响应客户端["
						+ client.keyFor(selector).hashCode() + "]数据 :["
						+ logResponseString + "]");
			} catch (IOException e) {
				e.printStackTrace();
				try {
		...
		public void run() {
			try {
				...
				while ((len = client.read(tmp)) > 0) {
					...
					System.arraycopy(tmp.array(), 0, data, data.length - len,
							len);
					tmp.rewind();
				}
				if (data.length == 0) {
