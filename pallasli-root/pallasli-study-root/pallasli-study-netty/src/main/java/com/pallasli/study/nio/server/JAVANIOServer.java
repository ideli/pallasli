package com.pallasli.study.nio.server;
...
public class JAVANIOServer {
	...
	private final Map<Integer, Integer> rejectedThreadCountMap = new ConcurrentHashMap<>();
			...
			try {
				...
				if (count < 1) {
					...
					if (r instanceof WriteClientSocketHandler) {
						...
						System.out
								.println("服务系统繁忙,客户端WriteClientSocketHandler["
										+ realThread.client + "]请求被拒绝处理！");
						SelectionKey selectionKey = realThread.client
								.keyFor(selector);
						...
						AtomicBoolean isWriting = isWritingMap
								.get(realThread.client);
						isWriting.set(false);
					} else if (r instanceof ReadClientSocketHandler) {
						ReadClientSocketHandler realThread = (ReadClientSocketHandler) r;
						System.out.println("服务系统繁忙,客户端ReadClientSocketHandler["
								+ realThread.client + "]请求被拒绝处理！");
						SelectionKey selectionKey = realThread.client
								.keyFor(selector);
						if (selectionKey != null) {
						...
						AtomicBoolean isReading = isReadingMap
								.get(realThread.client);
						isReading.set(false);
					} else {
						System.out.println("服务系统繁忙,系统线程["
								+ r.getClass().getName() + "]被拒绝处理！");
					}
				}
			} catch (InterruptedException e) {
				System.out.println("RejectedExecutionHandler处理发生异常："
						+ e.getMessage());
			}
		}
	};

	private final ThreadPoolExecutor threadPool = new ThreadPoolExecutor(30,
			50, 300, TimeUnit.MILLISECONDS,
			new ArrayBlockingQueue<Runnable>(5), rejectedExecutionHandler);

	private final Map<SocketChannel, AtomicBoolean> isReadingMap = new ConcurrentHashMap<SocketChannel, AtomicBoolean>();
	private final Map<SocketChannel, AtomicBoolean> isWritingMap = new ConcurrentHashMap<SocketChannel, AtomicBoolean>();
					...
					while (iterator.hasNext()) {
						...
						try {
							if (selectedKey.isReadable()) {
								...
								isReading.set(true);
								threadPool.execute(new ReadClientSocketHandler(
										socketChannel));
							} else if (selectedKey.isWritable()) {
								Object responseMessage = selectedKey
										.attachment();
								SocketChannel socketChannel = (SocketChannel) selectedKey
										.channel();
								selectedKey.interestOps(SelectionKey.OP_READ);
								threadPool
										.execute(new WriteClientSocketHandler(
												socketChannel, responseMessage));
							} else if (selectedKey.isAcceptable()) {
								ServerSocketChannel ssc = (ServerSocketChannel) selectedKey
										.channel();
	...
	private class WriteClientSocketHandler implements Runnable {
		...
		private WriteClientSocketHandler(SocketChannel client,
		...
		public void run() {
			try {
				...
				ByteBuffer data = ByteBuffer
						.allocate(responseByteData.length + 4);
				...
				System.out.println("server响应客户端[" + client.getRemoteAddress()
						+ "]数据 :[" + new String(logResponseString) + "]");
			} catch (Exception e) {
				try {
					System.out.println("server响应客户端["
							+ client.getRemoteAddress() + "]数据 异常["
							+ e.getMessage() + "]");
	...
	private class ReadClientSocketHandler implements Runnable {
		private final SocketChannel client;
		private final ByteBuffer dataLen = ByteBuffer.allocate(4);
		...
		public void run() {
			try {
				...
				System.out.println(Thread.currentThread().getId()
						+ "服务器接收到客户端[" + client.getRemoteAddress() + "]数据 :["
						+ readData + "]");

				// dosomthing
				byte[] response = ("response" + readData.substring(0, 3))
						.getBytes();
				AtomicBoolean isWriting = isWritingMap.get(client);
				if (isWriting == null) {
				...
				try {
					...
					System.out
							.println("客户端[" + client + "]关闭了连接,原因[" + e + "]");
					if (client != null) {
						client.close();
					}

				} catch (IOException e1) {
					System.out.println("客户端[" + client + "]关闭异常"
							+ e1.getMessage());
				}
			} finally {
