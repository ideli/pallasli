...
public class Client1 {
	...
	private final Selector selctor = getSelector();

	private final List<Object> messageQueue = new LinkedList<>();

	private final Set<String> set = new HashSet<String>();
	private final ThreadPoolExecutor threadPool = new ThreadPoolExecutor(3, 5,
			...
			public void run() {
				while (run) {
					try {
						...
						Iterator<SelectionKey> iterator = selctor
						...
						while (iterator.hasNext()) {
							...
							if (selectionKey.isConnectable()) {
								...
								SocketChannel writeSocketChannel = (SocketChannel) selectionKey
										.channel();
								...
								SocketChannel readSocketChannel = (SocketChannel) selectionKey
										.channel();
								...
								if ((len = readSocketChannel.read(tmp)) > 0) {
									data = Arrays.copyOf(data, data.length
											+ len);
									System.arraycopy(tmp.array(), 0, data,
											data.length - len, len);
									tmp.rewind();
								}
								if (data.length > 0) {
									String receiveData = new String(data);
									set.add(receiveData.substring(receiveData
											.length() - 3));
