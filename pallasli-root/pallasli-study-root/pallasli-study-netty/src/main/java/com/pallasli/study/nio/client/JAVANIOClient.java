package com.pallasli.study.nio.client;
...
public class JAVANIOClient {
	...
	private final ThreadPoolExecutor threadPool = new ThreadPoolExecutor(5, 10,
			200, TimeUnit.MILLISECONDS, new ArrayBlockingQueue<Runnable>(20));
					...
					while (iterator.hasNext()) {
						...
						if (selectionKey.isReadable()) {
							...
							if (isReading.get()) {
								...
								isReading.set(true);
								threadPool.execute(new ReceiveMessageHandler(
										socketChannel));
							}
							...
							threadPool.execute(new SendMessageHandler(
									socketChannel, requestMessage));
						} else if (selectionKey.isConnectable()) {
							SocketChannel sc = (SocketChannel) selectionKey
									.channel();
							sc.finishConnect();
		...
		public void run() {
			try {
				...
				if (requestMessage instanceof byte[]) {
					...
					requestMessageByteData = ((String) requestMessage)
							.getBytes();
				}
				if (requestMessageByteData == null
						|| requestMessageByteData.length == 0) {
					System.out.println("客户端发送的数据为空");
				} else {

					ByteBuffer data = ByteBuffer
							.allocate(requestMessageByteData.length + 4);
					...
					System.out.println("[" + sdf.format(date) + "]["
							+ Thread.currentThread().getId() + "]客户端发送数据：["
		...
		public void run() {
			try {
				...
				SimpleDateFormat sdf = new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss:SSS");
				System.out.println("[" + sdf.format(date) + "]["
						+ Thread.currentThread().getId() + "]客户端接收到服务器["
