...
public class HtmlParserTool {
	...
	// 方法。如果希望迭代网页节点的时候保留当前节点，则在节点条件满足的情况下返回 true；否则返回 false。HtmlParse 里提供了很多实现了
	...
	// 判断节点本身情况的 Filter 有
	// HasAttributeFilter：判读节点是否有特定属性；LinkStringFilter：判断节点是否是具有特定模式 (pattern)
	// url 的节点；
	// TagNameFilter：判断节点是否具有特定的名字；NodeClassFilter：判读节点是否是某个 HtmlParser 定义好的 Tag
	...
	public static void extracLinks(String url) {
		try {
			...
			OrFilte rorFilter = new OrFilter(
