...
public class StringUtils {
	...
	public static String replaceBetween(String source, String token,
			int beginIndex) {
		if (source.length() - 1 != beginIndex) {
			int endIndex = source.indexOf(token, beginIndex + 1);
			String interceptString = source.substring(beginIndex, endIndex);
	...
	public static String removeLastMark(String str, String mark) {
		if (str != null && mark != null && mark.length() != 0
				&& str.length() != 0) {
			int markStart = str.lastIndexOf(mark);
			int markLength = mark.length();
			str = str.substring(0, markStart)
					+ str.substring(markStart + markLength, str.length());
	...
	public static String replaceLastMark(String str, String mark, String replace) {
		if (str != null && mark != null && mark.length() != 0
			...
			int markLength = mark.length();
			str = str.substring(0, markStart) + replace
					+ str.substring(markStart + markLength, str.length());
		}
	...
	public static boolean updateSpellAndSimpleSpellOfTable(String spellColumn,
			String simpleSpellColumn, String chineseColumn, String table) {
		boolean rtn = false;
		StringBuffer sql = new StringBuffer();
		sql.append("update ").append(table).append(" set ").append(spellColumn)
				.append("=").append(getPinyin(chineseColumn)).append(",set ")
				.append(simpleSpellColumn).append("=")
