...
@WebFilter(filterName = "Servlet3Filter", urlPatterns = "/*", initParams = {
		@WebInitParam(name = "basePackage", value = "com.pallasli.study.servlet30.business,com.pallasli.study.servlet30.login") })
public class AnnotationHandleFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (uri.indexOf("!") == -1) {
			...
			MyWebServlet annotationInstance = clazz.getAnnotation(MyWebServlet.class);
			// 获取注解上配置的初始化参数数组
			MyWebInitParam[] initParamArr = annotationInstance.initParams();
			Map<String, String> initParamMap = new HashMap<String, String>();
			for (MyWebInitParam initParam : initParamArr) {
