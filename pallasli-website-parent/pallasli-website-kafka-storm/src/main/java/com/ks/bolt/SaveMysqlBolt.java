...
import org.apache.storm.shade.org.apache.curator.framework.recipes.locks.InterProcessMutex;
...
public class SaveMysqlBolt extends BaseBasicBolt {
	...
	private void saveMap(String sendpay, boolean isHasMem, String totalprice, String youhui) {
		if (isOpen) {
			if (checkMap()) {
				String value = memberMap.get(sendpay);// value =
														// count(id),sum(totalPrice),sum(totalPrice
														// -
														// youhui),count(distinct
														// memberid)
				if (value != null) {
					...
					value = 1 + "," + totalprice + "," + (Double.valueOf(totalprice) - Double.valueOf(youhui)) + ","
							+ (isHasMem ? 0 : 1);
				}
			...
			String value = sendpay + "_" + 1 + "," + totalprice + ","
					+ (Double.valueOf(totalprice) - Double.valueOf(youhui)) + "," + (isHasMem ? 0 : 1);
			cacheList.add(value);
		}
	}
				...
				for (Map.Entry<String, String> entry : tmpMap.entrySet()) {
					...
					if (id == 0) {// insert
						sBuffer.append(
								...
								.append(order_nums + "," + p_total_price + "," + y_total_price + "," + order_member
										+ ",'" + key + "')");
					} else {// update
						sBuffer.append("update total_order set order_nums=" + order_nums)
								.append(",p_total_price=" + p_total_price).append(",y_total_price=" + y_total_price)
								.append(",order_members=" + order_member).append(" where id=" + id);
