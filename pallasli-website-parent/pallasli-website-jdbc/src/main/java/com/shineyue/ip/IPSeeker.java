...
public class IPSeeker {
	...
	private static String IP_FILE = INSTALL_DIR + "/QQWry.Dat";
	...
	public void getTxtFile()
	{
		...
		{
			long i = 0;
			for(long l=ipBegin; l<=ipEnd; l +=7, i++)
			{
				//ipFile.seek(l);
				//读入索引中的起始IP
				byte [] ip = new byte [4];
				readIP(l, ip);				
				String bIP = Util.getIpStringFromBytes(ip);
				//System.out.print((i++) + "\t");// + l + "\t");
				System.out.print(bIP + (bIP.length()<8 ? "\t\t" : "\t"));
				
				//IP记录的偏移量
				long pos = readLong3(l + 4);
				//结束IP地址
				readIP(pos, ip);
				System.out.print(Util.getIpStringFromBytes(ip) + "\t");
				//获取IP位置					
				IPLocation ipl = getIPLocation(pos);
				System.out.println(ipl.country + " " + ipl.area);		
			}
			System.out.println("\n\nIP数据库共有数据 ： " + i + " 条\n");
		}
		catch(Exception e)
