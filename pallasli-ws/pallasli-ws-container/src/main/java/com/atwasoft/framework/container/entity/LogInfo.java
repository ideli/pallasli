...
import com.atwasoft.framework.container.ContainerEnv;
...
public class LogInfo 
{
	...
	{
		return line.indexOf("<font color='" + ContainerEnv.COLOR_RECEIVE + "'>") != -1;		
	}
	private static boolean isSend(String line)
	{
		return line.indexOf("<font color='" + ContainerEnv.COLOR_SEND + "'>") != -1;
	}
	private static boolean isError(String line)
	{
		return line.indexOf("<font color=\"" + ContainerEnv.COLOR_ERROR + "\">") != -1;
	}
	/**
	 * 向银行统计表里加入记录
	 * @param bank		银行编号
	 * @param recvCount	接受数据次数
	 * @param sendCount 发送数据次数
	 */
	...
	private void addTrans(int transCode, int transCount, int failedCount)
	{
		...
		{
			logTrans.put(transCode, new LogTrans(transCode, transCount, failedCount));
		}
	}
	/**
	 * 处理行数据，由读文件方法回调
	...
	{		
		...
		{			
			...
			if (index > 3)
			{
				int transCode = Integer.parseInt(line.substring(index-3, index));
				if (isError(line)) failedCount++;		
	 ...
	 * @param filePath	日志文件
	 * @param bank		银行编号
	 * @param method	统计方法，total:该银行所有请求
	 * 							recv :银行接受数据请求
	 * 							send :银行发送数据请求
	 * @return
	 */
	...
	{
		StringBuffer sb = new StringBuffer("<table border=\"0\" class=\"jtpsoft\">");
		try 
		{
		    BufferedReader br = new BufferedReader(
		    		new InputStreamReader(new FileInputStream(filePath),ContainerEnv.CHARSET));
	...
	public static String TransStat(String filePath, String transCode, String bz)
	{
		...
		try 
		{
			...
			while (line != null) 
			{
				int index = line.indexOf("交易)");
				if (index < 3) continue;
				if (transCode.equals("all") || line.substring(index-3, index).equals(transCode))
				{
					if ( bz.equals("1") ||
						(bz.equals("0") && isError(line)))
					{
						sb.append(line).append("\n");
					}
				}
	...
	public String BankToHtmlString()
	{
		...
		sb.append("<tr align=\"center\"><th>银行</th><th>请求数据次数</th><th>发送数据次数</th><th>合计次数</th></tr>");
		...
		while (itr.hasNext())
        {
            ...
            sb.append("<tr><td>").append(value.getBank()).append("</td>")
            	...
            	//.append(" onclick=\"filterBank('recv','").append(value.getBank()).append("')")
            	.append("\">").append(value.getRecvCount()).append("</td>")
            	.append("<td align=\"right\" class=\"changeColor\"")
            	//.append(" onclick=\"filterBank('send','").append(value.getBank()).append("')")
            	.append("\">").append(value.getSendCount()).append("</td>")
            	.append("<td align=\"right\" class=\"changeColor\"")
            	//.append(" onclick=\"filterBank('total','").append(value.getBank()).append("')")
            	.append("\">").append(value.getTotalCount()).append("</td>")
            	.append("</tr>");
        }
        sb.append("<tr><td>").append("合计&nbsp;&nbsp;").append(t_num).append("</td>")
    		.append("<td align=\"right\" class=\"changeColor\"")
    		//.append(" onclick=\"filterBank('recv','all')")
    		.append("\">").append(t_recv).append("</td>")
    		.append("<td align=\"right\" class=\"changeColor\"")
    		//.append(" onclick=\"filterBank('send','all')")
    		.append("\">").append(t_send).append("</td>")
    		.append("<td align=\"right\" class=\"changeColor\"")
    		//.append(" onclick=\"filterBank('total','all'")
    		.append(")\">").append(t_total).append("</td>")
    		.append("</tr></table>");
