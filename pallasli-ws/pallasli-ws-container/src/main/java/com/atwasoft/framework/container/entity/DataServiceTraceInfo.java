...
import com.atwasoft.framework.container.util.DateUtil;

public class DataServiceTraceInfo {
	private static final Log log = LogFactory.getLog(DataServiceTraceInfo.class);
	private static DataServiceTraceInfo self = null;
	private static int len = Runtime.getRuntime().availableProcessors() * ContainerEnv.getInstance().getAppPoolSize();
	public static enum TYPE{Console,Ef,Wasys3_gz,Wasys3_gd,Wasys3_cw,Common,Zj,Gz_tq,Gz_zy,Gz_nddq,Gz_hbj,Gz_zhgl,Gz_lscxtj,Gd_kfsxmhzdw,Gd_dhgl,Gd_dksq,Gd_bzj,Gd_nddq,Gd_lscxtj,Gd_phhs,
		       Cw_pzgl,Cw_zbgl,Cw_pzcx,Cw_yhdz,Cw_lszbgl,Cw_lspzcx,Cw_lsyhdz,Cw_bbgl,Cw_lsbbgl,Cw_pjgl,Report,OA,Hr,Cc,Lmk,Ai,Da,Be,Sms,Bt_tq,Bt_zy,Bt_nddq,Bt_hbj,Bt_zhgl,Bt_lscxtj,
		       Website,Ds,Sfk,Wxj,Fa,Im,Dkdb,Gzdw,Gzlxjs,Dascan,Ale,Fxyj,Zx,Crm,Resp};
	...
	public synchronized DataServiceTraceInfo getTrace(String transInfo, TYPE type)
	{
		return getTrace(new Date(), transInfo, type);
	}
	public synchronized DataServiceTraceInfo getTrace(Date transTime, String transInfo, TYPE type)	
	{
		if ( ++curIndex >= len)//循环数组
		{
			curIndex = 0;
		}
		DataServiceTraceInfo dsti = new DataServiceTraceInfo(transTime, transInfo, type);
	...
	public synchronized String getTraceInfo2Html()
	{
		...
		int logCounter=ContainerEnv.getInstance().getLogCounter();
	...
	{
		sb.append("<tr><td width=\"150\">[").append(i > 9 ? i : "0" + i).append("]")
		  .append(DateUtil.formatDate(dstis[i].getServerTime(), "yyyy-MM-dd HH:mm:ss")).append("&nbsp;</td><td>")
		  .append(dstis[i].getServerInfo()).append("</td></tr>");
		  
	}
	private void setTrace2Json(StringBuffer sb, int i)
	{
		sb.append("[\"").append(i > 9 ? i : "0" + i).append("\",\"")
		  .append(DateUtil.formatDate(dstis[i].getServerTime(), "yyyy-MM-dd HH:mm:ss")).append("\",\"")
		  .append(dstis[i].getServerInfo()).append("\"],");
	}
